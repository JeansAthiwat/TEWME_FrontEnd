...
const VideoPlayer = ({ videoObj, videoNumber, courseId }) => {
    return (
      <div className="playerWrapper">
        ...
        onProgress={(state) => localStorage.setItem(`videoProgress_${courseId}_${videoNumber}`, Math.max(localStorage.getItem("videoProgress"),state.playedSeconds))}
...
const VideoPage = () => {
    ...
    const [videoProgress, setVideoProgress] = useState(
      ...
      const [videoDuration, setVideoDuration] = useState(
        parseFloat(localStorage.getItem("videoDuration")) || 1 // default to 1 to avoid division by zero
      );
    ...
    useEffect(() => {
        ...
        const progressInterval = setInterval(() => {
          const progress = parseFloat(localStorage.getItem("videoProgress"));
          const duration = parseFloat(localStorage.getItem("videoDuration"));
